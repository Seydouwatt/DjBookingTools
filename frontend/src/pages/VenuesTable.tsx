...
function contactScore(venue: Venue): number {
  return (venue.instagram ? 1 : 0) + (venue.email ? 1 : 0) + (venue.facebook ? 1 : 0);
}
...
export const VenuesTable: React.FC = () => {
  const { venues, loading, filters, fetchVenues, setFilters, updateVenue, deleteVenue } = useVenueStore();
    ...
    () => [
      {
        ...
        header: 'Nom',
        cell: ({ row }) => (
          <Link to={`/venues/${row.original.id}`} className="text-primary-400 hover:underline font-medium">
            {row.original.name}
          </Link>
        ),
      },
      ...
      { accessorKey: 'category', header: 'Catégorie' },
      {
        ...
        cell: ({ row }) =>
          row.original.instagram ? (
            <a href={row.original.instagram} target="_blank" rel="noreferrer" className="text-pink-400 hover:underline text-sm">
              📷 Instagram
            </a>
      ...
      },
      {
        ...
        cell: ({ row }) =>
          row.original.email ? (
            <a href={`mailto:${row.original.email}`} className="text-blue-400 hover:underline text-sm">
              ✉ {row.original.email}
            </a>
          ) : <span className="text-gray-600">—</span>,
      },
      {
        ...
        cell: ({ row }) =>
          row.original.facebook ? (
            <a href={row.original.facebook} target="_blank" rel="noreferrer" className="text-blue-300 hover:underline text-sm">
              fb
            </a>
          ) : <span className="text-gray-600">—</span>,
      },
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <select
            ...
            onChange={(e) => updateVenue(row.original.id, { status: e.target.value as VenueStatus })}
      ...
      },
      {
        ...
        cell: ({ getValue }) => {
          ...
          return val ? <span className="text-sm text-gray-400">{new Date(val).toLocaleDateString('fr-FR')}</span> : <span className="text-gray-600">—</span>;
        },
      },
      {
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <button
            onClick={(e) => {
              e.stopPropagation();
              if (confirm('Supprimer ce lieu ?')) deleteVenue(row.original.id);
            }}
